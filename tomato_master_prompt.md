...
1. **Real-subprocess smoke test on the sandbox server (port 8767) under venv Python.** Note on "real models": **Phase 5a tests real signal-compute paths** (un-mocked from TestClient stubs in `test_endpoints.py`). **Real-loaded model weights are F.0 territory per spec Section 29 and are explicitly NOT required by Phase 5a.** The pre-F.0 sandbox skip on startup steps 4-7 (model loading) remains intact. Server boots cleanly. `/ready` returns 200. `/health`, `/info`, `/metrics` respond. The server must launch via `venv/Scripts/python.exe -m uvicorn tomato_sandbox.api.server:app --host 127.0.0.1 --port 8767` (NOT system Python; NOT in-process TestClient). **[CLARIFIED 2026-05-02 per DEC-047 — original wording said "with real model loading" which was ambiguous; resolved to (β) interpretation: un-mocked compute paths, not loaded weights.]**
...
2. **Real-image POST `/predict` end-to-end test.** A real leaf image (not synthetic noise; ideally an image from `data/specialist/model3/` or one of the held-out evaluation samples) goes through the full pipeline: `validate → IQA → preprocess → signals (A, B, C) → classifier → conformal → tier_assignment → response_builder`. Response shape matches spec Section 16.2 schema. Response is **NOT a structured error** (`422 IQA_REJECTED`, `503 SERVER_OVERLOAD`, etc.) UNLESS the test image legitimately triggers that path (an off-domain photo legitimately rejected by IQA does count, but every leaf image being rejected does not).
   ...
   **Acceptance under degraded-mode ((β) interpretation per DEC-047):** Tier 4B from all-signals-failed-degraded-mode IS a spec-compliant S16.2 response (Tier 4B is a valid tier label per Section 14, **not** a top-level error). Phase 5a CLOSES when the response includes a valid `tier.label` value with no top-level `error` field — regardless of whether the tier is high-confidence or degraded. Real-prediction validation (high-confidence non-degraded predictions on real-loaded model weights) is Phase F.0 territory.
